...
/*
    Create Tune Button for FTdx101 using FM-N mode.
...
#define get_Mode "MD0;"                 // define request Mode command
  ...
  if (val == val2) {                      // when no bounce
    if (val == HIGH) {                    // the Tune button is pressed, and no bounce
      if (buttonpress == false) {         // input and output CAT commands only once when button pressed (it is in loop)
        ...
        send_command(get_Mode);           // send the request Mode Command to radio, call send routine
        get_response();                   // receive the present (mode) setting from radio
        ...
        if (tune == true) {               // valid power/mode info is received from radio, we can set tune mode & power now
          send_command("MD0B;");          // set mode to FM-N, call send routine
